...
//
// Compañía y cobertura NO van como campos aparte: multipart no permite meter campos
// custom adentro del objeto de un archivo, y mandarlos como arrays de texto en
// paralelo corre el riesgo de desalinearse con "images" (igual que pasó con los
// archivos repetidos). En cambio, van codificados en el propio "name" del archivo
// (p. ej. "BSE__GLOBAL - anual__OPP-9094.png") — así quedan garantizados al mismo
// nivel que esa imagen puntual (name es hermano de data/mime en cada item de
// images.files), sin depender de correlacionar dos arrays por índice. Del lado de
// Make se sacan con un split() sobre "name" dentro de la misma iteración.
...
// carácter sería ambiguo partirlo del lado de Make.
function buildFilename(raw, opportunity) {
  const cobertura = raw.cobertura || raw.name
  return `${raw.compania}__${cobertura}__${opportunity.oppNumber}.png`
}
