...
// real), así que `limit: 1` alcanza para traer el último. `text_body` ya viene en texto
// plano (sin el HTML de `body`), listo para mostrar tal cual.
const LATEST_UPDATE_QUERY = `
  query GetLatestUpdate($itemId: ID!) {
    items(ids: [$itemId]) {
      updates(limit: 1) {
        text_body
        created_at
...
export async function fetchLatestUpdate(itemId) {
